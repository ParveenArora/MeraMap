...
./$FILE_POSTGRE_PATCH #>> $LOGFILE_INST 2>&1         #Script to apply required configuration to user's default configuration of PostGreSQL.
su -c ./$FILE_POSTGRE_DB >> $LOGFILE_INST 2>&1            #Script to check the version of ubuntu, to create database and user of postgresql and to enable creatlang.
./$SCRIPT_OSM2PGSQL >> $LOGFILE_INST 2>&1           #Script to load osm data into pgsql.
